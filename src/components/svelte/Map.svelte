<script lang="ts">
  ...
  function startMarkerDrag(
    ...
    function onEnd() {
      ...
      const originalPos = buildings.find(
        (b) => b.building_name === buildingName,
      );
  ...
  async function saveBuildingPosition(
    ...
    try {
      const response = await fetch(
        `/api/building-position?building=${encodeURIComponent(buildingName)}`,
        {
          method: "PUT",
      ...
      {#if building.lat && building.lon}
        ...
        >
          <div
            ...
            title={adminAuthStore.isAdmin
              ? "Drag to edit position"
              : building.building_name}
            style:cursor={adminAuthStore.isAdmin && !isDragging
              ? "grab"
              : isDragging
                ? "grabbing"
                : "pointer"}
            onpointerdown={(e) =>
              startMarkerDrag(e, building.building_name, displayPos[0], displayPos[1])}
          >
            <University size="20" />
            <div
              class="pin-label"
...
<style>
  ...
  @keyframes pulse-edit {
    0%,
    100% {
      box-shadow: 0 0 0.5rem hsl(39, 84%, 53%),
        0 2px 0.5rem rgba(0, 0, 0, 0.4);
    }
    50% {
      box-shadow: 0 0 1rem hsl(39, 84%, 63%),
        0 2px 0.75rem rgba(0, 0, 0, 0.5);
    }
  }
